...
% obtained while using ISPTWSVM (for 
% linear case). 
%---------------------------------------------
	...
	for j = 1: size(tau, 1)
		...
		for k = 1: 13								%Iterate on 13 values of c1
			...
			for q = 1: 13							%Iterate on 13 values of c3
				c3 = c3*10;
				[accuracy, non_zero_dual_variables, training_time, lambda] = ISPTWSVM(X1_Train, X2_Train, X_Test, Y_Test, c1, c3, epsilon(i), tau(j));
				if(maxx < accuracy)
					maxx = accuracy;
					finalc1 = c1;
					finalc3 = c3;
		...
		[accuracy, non_zero_dual_variables, training_time, lambda] = ISPTWSVM(X1_Train, X2_Train, X_Test, Y_Test, finalc1, finalc3, epsilon(i), tau(j));
		temp = [temp accuracy];
		tempc = [tempc finalc1];
		% if(tau(j) == 0.5)
