...
% obtained while using ISPTWSVM (for non-
% linear case). 
%---------------------------------------------
...
%--------Setting ranges for epsilon, tau, values of c1 and c3, and gamma----------
...
for i = 1: size(epsilon, 1)
	...
	tempgamma = [];
	for j = 1: size(tau, 1)
		c1 = power(10, -7);										%Calculating maximum accuracy for each (epsilon, tau) combination
		maxx = 0;
		for k = 1: 13											%Iterate on 13 values of c1
			...
			for q = 1: 13										%Iterate on 13 values of c3
				...
				for l = 1: 10
					gamma = gamma*10;
					[accuracy, non_zero_dual_variables, training_time, lambda] = ISPTWSVM_Kernel(X1_Train, X2_Train, X_Test, Y_Test, c1, c3, gamma, epsilon(i), tau(j));
					if(maxx < accuracy)
						maxx = accuracy;
						finalc1 = c1;
						finalc3 = c3;
		...
		[accuracy, non_zero_dual_variables, training_time, lambda] = ISPTWSVM_Kernel(X1_Train, X2_Train, X_Test, Y_Test, finalc1, finalc3, finalgamma, epsilon(i), tau(j));
		temp = [temp accuracy];
		tempc = [tempc finalc1];
